...
author: xkcoding
pubDatetime: 2017-07-12T03:26:43.000Z
slug: jdk-map-source-explore
featured: false
draft: false
tags:
  - java
  - 源码分析
description: "Map.Entry 明明是个接口，为什么能直接调用 getKey() 和 getValue()？从一个朋友的疑问出发，深入 HashMap 源码，揭秘 entrySet() 返回的其实是内部类 EntrySet，而真正干活的是实现了 Map.Entry 的 Node 类。一次完整的 Java 多态实战分析，帮你理解接口、实现类、内部类之间的关系。"
---
...
1. 首先，我们看 `map` 对象，这个 `map` 对象是 `HashMap` 的一个实例，然后下面的 `Set set = map.entrySet();` 可以知道这其实用的 `HashMap` 实现的 `entrySet()` 方法，然后我们可以查看 `HashMap` 里 `entrySet()` 的源码
   ...
   ![](https://cdn.xkcoding.com/2017-07-12-14998277900006.jpg?x-oss-process=style/tag_compress)
