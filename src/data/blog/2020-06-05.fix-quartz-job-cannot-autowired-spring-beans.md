...
tags:
  - spring-boot
  - java
description: "在 Quartz Job 里 @Autowired 结果 NPE？别急着怀疑人生。这是因为 Job 实例由 Quartz 内部创建，压根不在 Spring 容器管辖范围内。本文从 SchedulerFactoryBean 源码入手分析问题根因，提供两种解法：自定义 JobFactory 配合 AutowireCapableBeanFactory，或者用 SpringUtil 工具类手动获取 Bean。文末还揭秘了 Spring Boot 2.x 为什么默认就能注入——因为它用了 SpringBeanJobFactory。"
---
